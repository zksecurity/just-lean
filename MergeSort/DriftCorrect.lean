...
theorem mergeRuns_spec (v s : A) (lo mid hi : UInt64) hv hs hvsz hssz hlo hmid
  ...
  unfold mergeRuns
  dsimp only
  obtain ⟨M, MF⟩ := mergeKernelS_spec lo mid hi v s hssz hv hs hlo hmid hL hR
  ...
  refine ⟨?_, fun x hx => C2 x hx, fun x hx => MF x hx⟩
  rw [← M]
  apply slice_congr
  intro t ht
...
theorem smallSortRest_spec (v s : A) (lo mid hi pre : UInt64) hv hs hvsz hssz h1 h2
    ...
    (∀ x, (x < lo.toNat ∨ hi.toNat ≤ x) → (smallSortRest v s lo mid hi pre hv hs hvsz hssz h1 h2).1.1.at' x = v.at' x) ∧
    ∀ x, (x < lo.toNat ∨ hi.toNat ≤ x) → (smallSortRest v s lo mid hi pre hv hs hvsz hssz h1 h2).1.2.at' x = s.at' x := by
  ...
  have G1 := (copyRange_spec mid v (mid.toNat - (lo + pre).toNat) (lo + pre) s hssz (by omega) (by omega) rfl).2
  ...
  have G3 := (copyRange_spec hi v (hi.toNat - (mid + pre).toNat) (mid + pre) S2 (by rw [hS2s, hS1s]; exact hssz) hv
    (by rw [hS2s, hS1s]; exact hs) rfl).2
...
theorem smallSort_spec (v s : A) (lo hi : UInt64) hv hs hvsz hssz hlo :
  ...
  · rename_i h2
    ...
    · -- len ≥ 16: two sort8 = four sort4 in place + two merges of 4+4 into s
      ...
      obtain ⟨RS, RP, RF, RG⟩ := R
      refine ⟨RS, ?_, fun x hx => ?_, fun x hx => ?_⟩
      · refine RP.trans ?_
      ...
      · rw [RG x hx, FM2 x (by rw [m8]; omega), FM1 x (by rw [e8]; omega)]
    · split
      · -- 8 ≤ len < 16: sort4 on each half in place, copied into s
        ...
        have G1 := (copyRange_spec (lo + 4) V1 ((lo + 4).toNat - lo.toNat) lo s hssz (by rw [hV1s]; omega) (by omega) rfl).2
        rw [hS1] at C1 G1
        dsimp only at C1 G1 ⊢
        ...
        have G2 := (copyRange_spec (lo + (hi - lo) / 2 + 4) V2 ((lo + (hi - lo) / 2 + 4).toNat - (lo + (hi - lo) / 2).toNat)
          (lo + (hi - lo) / 2) S1 (by rw [hS1s]; exact hssz) (by rw [hV2s, hV1s]; omega) (by rw [hS1s]; omega) rfl).2
        rw [hS2] at C2 C2f G2
        dsimp only at C2 C2f G2 ⊢
        ...
        · rw [RG x hx, G2 x (by rw [m4]; omega), G1 x (by rw [e4]; omega)]
      · -- 2 ≤ len < 8: one element of each half copied into s
        ...
        have G1 := (copyRange_spec (lo + 1) v ((lo + 1).toNat - lo.toNat) lo s hssz (by omega) (by omega) rfl).2
        rw [hS1] at C1 G1
        dsimp only at C1 G1 ⊢
        ...
        have G2 := (copyRange_spec (lo + (hi - lo) / 2 + 1) v ((lo + (hi - lo) / 2 + 1).toNat - (lo + (hi - lo) / 2).toNat)
          (lo + (hi - lo) / 2) S1 (by rw [hS1s]; exact hssz) (by omega) (by rw [hS1s]; omega) rfl).2
        rw [hS2] at C2 C2f G2
        dsimp only at C2 C2f G2 ⊢
        ...
        have R := smallSortRest_spec v S2 lo (lo + (hi - lo) / 2) hi 1 hv (by rw [hS2s, hS1s]; exact hs) hvsz
          ...
          (by simp only [UInt64.toNat_ofNat, Nat.reducePow, Nat.reduceMod]; exact sorted_of_length_le_one _ (by simp))
        obtain ⟨RS, RP, RF, RG⟩ := R
        refine ⟨RS, ?_, fun x hx => RF x hx, fun x hx => by rw [RG x hx, G2 x (by rw [m1]; omega), G1 x (by rw [e1]; omega)]⟩
        refine RP.trans ?_
...
theorem filter_single_pos (p : UInt64 → Bool) (x : UInt64) (h : p x = true) : [x].filter p = [x] := by simp [h]
theorem filter_single_neg (p : UInt64 → Bool) (x : UInt64) (h : p x = false) : [x].filter p = [] := by simp [h]
theorem filter_not_single_pos (p : UInt64 → Bool) (x : UInt64) (h : p x = true) : [x].filter (fun y => !p y) = [] := by simp [h]
theorem filter_not_single_neg (p : UInt64 → Bool) (x : UInt64) (h : p x = false) : [x].filter (fun y => !p y) = [x] := by simp [h]

/-- The scan invariant: the left part so far at the front of `s[lo, ..)`, the right part so far at the
    back of `s[.., hi)` in reverse. -/
theorem partScan_spec (p : UInt64 → Bool) (v : A) (lo hi : UInt64) :
    ∀ (m : Nat) (s : A) (i nl sv : UInt64) hv hs hssz hi1 hi2 hnl hsv, m = hi.toNat - i.toNat →
    s.slice lo.toNat nl.toNat = (v.slice lo.toNat (i.toNat - lo.toNat)).filter p →
    (s.slice (hi.toNat - (i.toNat - lo.toNat - nl.toNat)) (i.toNat - lo.toNat - nl.toNat)).reverse
        = (v.slice lo.toNat (i.toNat - lo.toNat)).filter (fun y => !p y) →
    (partScan p v s lo hi i nl sv hv hs hssz hi1 hi2 hnl hsv).1.buf.slice lo.toNat
        (partScan p v s lo hi i nl sv hv hs hssz hi1 hi2 hnl hsv).1.nl.toNat
      = (v.slice lo.toNat (hi.toNat - lo.toNat)).filter p ∧
    ((partScan p v s lo hi i nl sv hv hs hssz hi1 hi2 hnl hsv).1.buf.slice
        (lo.toNat + (partScan p v s lo hi i nl sv hv hs hssz hi1 hi2 hnl hsv).1.nl.toNat)
        (hi.toNat - lo.toNat - (partScan p v s lo hi i nl sv hv hs hssz hi1 hi2 hnl hsv).1.nl.toNat)).reverse
      = (v.slice lo.toNat (hi.toNat - lo.toNat)).filter (fun y => !p y) ∧
    ∀ x, (x < lo.toNat ∨ hi.toNat ≤ x) → (partScan p v s lo hi i nl sv hv hs hssz hi1 hi2 hnl hsv).1.buf.at' x = s.at' x := by
  intro m
  induction m using Nat.strongRecOn with
  | _ m ih =>
  intro s i nl sv hv hs hssz hi1 hi2 hnl hsv hm HL HR
  rw [partScan]
  split
  · rename_i h
    ...
    dsimp only
    have esv : (sv - 1).toNat = sv.toNat - 1 := UInt64.toNat_sub_of_le _ _ (UInt64.le_iff_toNat_le.mpr (by simp; omega))
    ...
    have hg : v.get i (by omega) = v.at' i.toNat := get_eq_at' _ _ _
    ...
    by_cases hgl : p (v.at' i.toNat) = true
    · have hgl' : p (v.get i (by omega)) = true := by rw [hg]; exact hgl
      have en : (nl + (if p (v.get i (by omega)) then (1 : UInt64) else 0)).toNat = nl.toNat + 1 := by
        rw [if_pos hgl']; exact toNat_add_of_lt _ _ (by simp; omega)
      have eidx : ((if p (v.get i (by omega)) then lo else sv - 1) + nl).toNat = lo.toNat + nl.toNat := by
        rw [if_pos hgl', e1]
      have H := ih (hi.toNat - (i + 1).toNat) (by omega)
        (s.set ((if p (v.get i (by omega)) then lo else sv - 1) + nl) (v.get i (by omega)) (by rw [eidx]; omega))
        (i + 1) (nl + (if p (v.get i (by omega)) then (1 : UInt64) else 0)) (sv - 1)
        hv (by simp; exact hs) (by simp; exact hssz) (by omega) (by omega) (by rw [en, ei]; omega) (by rw [esv, ei]; omega) rfl
        (by rw [en, ei, slice_succ, at'_set, if_pos eidx, slice_set_of_not_mem _ _ _ _ _ _ (Or.inr (by rw [eidx]; omega)), HL, hsl,
            List.filter_append, filter_single_pos _ _ hgl, hg])
        (by rw [en, ei, hsl, List.filter_append, filter_not_single_pos _ _ hgl, List.append_nil, ← HR,
    ...
    · have hglf : p (v.at' i.toNat) = false := by simpa using hgl
      have hgl' : ¬ p (v.get i (by omega)) = true := by rw [hg]; simp [hglf]
      have en : (nl + (if p (v.get i (by omega)) then (1 : UInt64) else 0)).toNat = nl.toNat := by
        rw [if_neg hgl']; simp
      have eidx : ((if p (v.get i (by omega)) then lo else sv - 1) + nl).toNat = hi.toNat - 1 - (i.toNat - lo.toNat - nl.toNat) := by
        rw [if_neg hgl', toNat_add_of_lt _ _ (by rw [esv]; omega), esv]; omega
      have H := ih (hi.toNat - (i + 1).toNat) (by omega)
        (s.set ((if p (v.get i (by omega)) then lo else sv - 1) + nl) (v.get i (by omega)) (by rw [eidx]; omega))
        (i + 1) (nl + (if p (v.get i (by omega)) then (1 : UInt64) else 0)) (sv - 1)
        hv (by simp; exact hs) (by simp; exact hssz) (by omega) (by omega) (by rw [en, ei]; omega) (by rw [esv, ei]; omega) rfl
        (by rw [en, ei, hsl, List.filter_append, filter_single_neg _ _ hglf, List.append_nil, ← HL,
            slice_set_of_not_mem _ _ _ _ _ _ (Or.inr (by rw [eidx]; omega))])
        (by rw [en, ei, hsl, List.filter_append, filter_not_single_neg _ _ hglf,
...
theorem stablePartition_spec (p : UInt64 → Bool) (a b : A) (lo hi : UInt64) ha hb hbsz hlo :
    (stablePartition p a b lo hi ha hb hbsz hlo).1.buf.slice lo.toNat (hi.toNat - lo.toNat) =
      (a.slice lo.toNat (hi.toNat - lo.toNat)).filter p ++ ((a.slice lo.toNat (hi.toNat - lo.toNat)).filter (fun y => !p y)).reverse ∧
    (stablePartition p a b lo hi ha hb hbsz hlo).1.nl.toNat = ((a.slice lo.toNat (hi.toNat - lo.toNat)).filter p).length ∧
    ∀ x, (x < lo.toNat ∨ hi.toNat ≤ x) → (stablePartition p a b lo hi ha hb hbsz hlo).1.buf.at' x = b.at' x := by
  unfold stablePartition
  dsimp only
  have P := partScan_spec p a lo hi (hi.toNat - lo.toNat) b lo 0 hi ha hb hbsz (by omega) hlo (by simp) (by simp) rfl (by simp) (by simp)
  generalize hP : partScan p a b lo hi lo 0 hi ha hb hbsz (by omega) hlo (by simp) (by simp) = R at P
  rcases R with ⟨⟨B1, nl⟩, hB1s, _, hnl⟩
  dsimp only at P hnl ⊢
  obtain ⟨A, B, C⟩ := P
  refine ⟨?_, by rw [← A, length_slice], C⟩
  have hnl' : nl.toNat ≤ hi.toNat - lo.toNat := by simpa using hnl
  have hsplit : B1.slice lo.toNat (hi.toNat - lo.toNat) =
      B1.slice lo.toNat nl.toNat ++ B1.slice (lo.toNat + nl.toNat) (hi.toNat - lo.toNat - nl.toNat) := by
    rw [← slice_add]; congr 1; omega
  rw [hsplit, A, ← B, List.reverse_reverse]

/-- The partition by pivot comparison, in the `leftPart`/`rightPart` form the quicksort proof uses. -/
theorem stablePartitionLt_spec (pivot : UInt64) (a b : A) (lo hi : UInt64) ha hb hbsz hlo :
    (stablePartition (ltPivot pivot) a b lo hi ha hb hbsz hlo).1.buf.slice lo.toNat (hi.toNat - lo.toNat) =
      leftPart false pivot (a.slice lo.toNat (hi.toNat - lo.toNat)) ++ (rightPart false pivot (a.slice lo.toNat (hi.toNat - lo.toNat))).reverse ∧
    (stablePartition (ltPivot pivot) a b lo hi ha hb hbsz hlo).1.nl.toNat = (leftPart false pivot (a.slice lo.toNat (hi.toNat - lo.toNat))).length ∧
    ∀ x, (x < lo.toNat ∨ hi.toNat ≤ x) → (stablePartition (ltPivot pivot) a b lo hi ha hb hbsz hlo).1.buf.at' x = b.at' x :=
  stablePartition_spec (ltPivot pivot) a b lo hi ha hb hbsz hlo

theorem stablePartitionLe_spec (pivot : UInt64) (a b : A) (lo hi : UInt64) ha hb hbsz hlo :
    (stablePartition (lePivot pivot) a b lo hi ha hb hbsz hlo).1.buf.slice lo.toNat (hi.toNat - lo.toNat) =
      leftPart true pivot (a.slice lo.toNat (hi.toNat - lo.toNat)) ++ (rightPart true pivot (a.slice lo.toNat (hi.toNat - lo.toNat))).reverse ∧
    (stablePartition (lePivot pivot) a b lo hi ha hb hbsz hlo).1.nl.toNat = (leftPart true pivot (a.slice lo.toNat (hi.toNat - lo.toNat))).length ∧
    ∀ x, (x < lo.toNat ∨ hi.toNat ≤ x) → (stablePartition (lePivot pivot) a b lo hi ha hb hbsz hlo).1.buf.at' x = b.at' x :=
  stablePartition_spec (lePivot pivot) a b lo hi ha hb hbsz hlo
