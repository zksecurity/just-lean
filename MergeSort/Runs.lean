...
def mergeRuns (w : Nat) (hw : 0 < w) (l : List UInt64) : List UInt64 :=
  ...
  else merge le64 (l.take w) ((l.drop w).take w) ++ mergeRuns w hw (l.drop (2 * w))
...
def ChunkSorted (w : Nat) (hw : 0 < w) (l : List UInt64) : Prop :=
  if _h : l.length ≤ w then Sorted le64 l
  else Sorted le64 (l.take w) ∧ ChunkSorted w hw (l.drop w)
termination_by l.length
decreasing_by simp; omega
...
theorem mergeRuns_of_lt {w : Nat} (hw) (l : List UInt64) (h : ¬ l.length ≤ w) :
    mergeRuns w hw l = merge le64 (l.take w) ((l.drop w).take w) ++ mergeRuns w hw (l.drop (2 * w)) := by
  rw [mergeRuns, dif_neg h]

theorem chunkSorted_of_length_le {w : Nat} (hw) {l : List UInt64} (h : l.length ≤ w) :
    ChunkSorted w hw l ↔ Sorted le64 l := by
  rw [ChunkSorted, dif_pos h]

theorem chunkSorted_of_lt {w : Nat} (hw) {l : List UInt64} (h : ¬ l.length ≤ w) :
    ChunkSorted w hw l ↔ Sorted le64 (l.take w) ∧ ChunkSorted w hw (l.drop w) := by
  rw [ChunkSorted, dif_neg h]

theorem sorted_of_length_le_one (l : List UInt64) (h : l.length ≤ 1) : Sorted le64 l := by
  match l, h with
  | [], _ => simp [Sorted]
  | [x], _ => simp [Sorted]
...
theorem mergeRuns_perm {w : Nat} (hw) (l : List UInt64) : (mergeRuns w hw l).Perm l := by
  ...
  | case2 l h ih =>
    ...
    calc (merge le64 (l.take w) ((l.drop w).take w) ++ mergeRuns w hw (l.drop (2 * w))).Perm
          ((l.take w ++ (l.drop w).take w) ++ l.drop (2 * w)) := (merge_perm le64 _ _).append ih
      _ = l := by rw [List.append_assoc, e]

theorem le64_trans : ∀ a b c, le64 a b = true → le64 b c = true → le64 a c = true :=
  fun a b c h1 h2 => by simpa using UInt64.le_trans (by simpa using h1) (by simpa using h2)
theorem le64_total : ∀ a b, le64 a b = true ∨ le64 b a = true :=
  fun a b => by simpa using UInt64.le_total a b
...
theorem chunkSorted_mergeRuns {w : Nat} (hw : 0 < w) (l : List UInt64) (hl : ChunkSorted w hw l) :
  ...
  | case2 l h ih =>
    ...
    have hB : Sorted le64 ((l.drop w).take w) ∧ ChunkSorted w hw (l.drop (2 * w)) := by
    ...
    have hAB : Sorted le64 (merge le64 (l.take w) ((l.drop w).take w)) := merge_sorted le64 le64_trans le64_total hA hB.1
    have ihM := ih hB.2
    ...
    · -- the remainder is empty
      ...
      rw [hM, mergeRuns_of_length_le hw [] (by simp), List.append_nil,
        chunkSorted_of_length_le _ (by rw [(merge_perm le64 _ _).length_eq]; simp; omega)]
      exact hAB
    · have hlenAB : (merge le64 (l.take w) ((l.drop w).take w)).length = 2 * w := by
        rw [(merge_perm le64 _ _).length_eq]; simp; omega
...
theorem sorted_of_chunkSorted {w : Nat} (hw) {l : List UInt64} (h : l.length ≤ w) (hl : ChunkSorted w hw l) :
    Sorted le64 l := (chunkSorted_of_length_le hw h).mp hl
